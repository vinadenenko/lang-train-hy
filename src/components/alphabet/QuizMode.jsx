...
function pickLetter(progress) {
  const weighted = ALPHABET.flatMap(letter => {
    ...
    if (!p) return [letter, letter, letter];
    if (p.mastered) return [letter];
...
export default function QuizMode({ progress, onRecord, lang, s }) {
  ...
  const sound = current.sounds[lang] ?? current.sounds.en;

  const prompt = quizType === 'letter-to-translit'
    ? { label: s.typeTranslit, display: current.upper, placeholder: s.placeholderTranslit }
    : { label: s.typeLetter,   display: current.translit, placeholder: s.placeholderLetter };

  return (
    <div className="max-w-md mx-auto">
      {/* Streak */}
      <div className="flex justify-between items-center mb-6 text-sm text-slate-500">
        <span>{s.streak}: <strong className="text-indigo-600">{streak}</strong></span>
        <button
        ...
        >
          {quizType === 'letter-to-translit' ? s.switchToTranslit : s.switchToLetter}
        </button>
      </div>
      ...
      `}>
        ...
        {feedback && (
          <div className="mt-4 text-sm">
            {feedback === 'correct'
              ? <span className="text-emerald-700 font-semibold">{s.correct}</span>
              : <span className="text-red-700 font-semibold">
                  {s.wrong} <span className="font-mono">
                    {quizType === 'letter-to-translit' ? current.translit : current.upper}
                  </span>
                </span>
            }
            <div className="text-slate-500 text-xs mt-1">{current.name} · {sound}</div>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          `}
        >
          {feedback ? s.next : s.check}
        </button>
      </div>
      <p className="text-xs text-slate-400 mt-2 text-center">{s.enterHint}</p>
    </div>
  );
}
